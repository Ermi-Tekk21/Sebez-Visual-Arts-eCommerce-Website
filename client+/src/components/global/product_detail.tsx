...
const ProductDetail: React.FC<ProductDetailProps> = ({ isOpen, onClose, product }) => {
  ...
  const isAdded = useMemo(() => {
    return cart.cartProducts.some((prod) => prod.id === product._id);
    ...
    <div className="fixed inset-0 p-4 flex justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto font-[sans-serif]">
      <div className="w-full max-w-lg bg-white shadow-lg rounded-lg p-6 relative">
        ...
        <div className="flex gap-5">
          <Image src={product.imageUrl} alt={product.item_name} width={200} height={200} />
          <div>
            <span className="font-semibold text-green-900">{product.item_name.toUpperCase()}</span>
            <p className="my-5 text-sm font-thin">{product.description}</p>
            <div className="flex justify-between w-full">
              <span className="font-semibold text-xl text-green-900">ETB {product.price}</span>
              {isAuthenticated ? (
                <div className="bg-custom-green-b text-sm text-white px-3 py-2 rounded">
                  <button
                    onClick={() => {
                      isAdded ? cart.removeFromCart(product._id) : cart.addToCart(product);
                      onClose();
                    }}
                  >
